...
> **地图底图**：由 `bootstrap_service.sh` / `start_http_server.sh` 自动确保（中国 Shortbread + 中亚五国叠加）。无需再单独跑其它脚本；无网环境可设 `SKIP_MBTILES=1`。详见 [tools/prepare_offline_map.md](tools/prepare_offline_map.md)。
